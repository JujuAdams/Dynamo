...
enum __DYNAMO_EXPRESSION_TOKEN
{
    __NULL       = -1,
    __UNKNOWN    =  0,
    __IDENTIFIER =  1,
...
function __DynamoExpressionCompile(_string)
{
    var _tokens = [];
    ...
    var _read_start   = 0;
    var _state        = __DYNAMO_EXPRESSION_TOKEN.__UNKNOWN;
    var _next_state   = __DYNAMO_EXPRESSION_TOKEN.__UNKNOWN;
    var _last_byte    = 0;
    var _new          = false;
    ...
    repeat(buffer_get_size(_buffer))
    {
        ...
        _next_state = (_byte == 0)? __DYNAMO_EXPRESSION_TOKEN.__NULL : __DYNAMO_EXPRESSION_TOKEN.__UNKNOWN;
        _change_state = true;
        _new = false;
        
        switch(_state)
        {
            case __DYNAMO_EXPRESSION_TOKEN.__IDENTIFIER: //Identifier (variable/function)
                #region
                
                if (_byte == ord(")"))
                {
                    _next_state = __DYNAMO_EXPRESSION_TOKEN.__SYMBOL;
                }
                else if ((_byte > 32) && (_byte != ord("$"))) //Everything is permitted, except whitespace and a dollar sign
                {
                    _next_state = __DYNAMO_EXPRESSION_TOKEN.__IDENTIFIER;
                }
            ...
            case __DYNAMO_EXPRESSION_TOKEN.__STRING: //Quote-delimited String
                ...
                {
                    _next_state = __DYNAMO_EXPRESSION_TOKEN.__STRING; //Quote-delimited String
                }
                
                #endregion
            break;
            
            case __DYNAMO_EXPRESSION_TOKEN.__NUMBER: //Number
                #region
                
                if (_byte == 46) //.
                {
                    _next_state = __DYNAMO_EXPRESSION_TOKEN.__NUMBER;
                }
                ...
                if (_state != _next_state)
                {
                    ...
                    {
                        __DynamoError("Error whilst converting expression value to real\n \n(", _error, ")");
                        return undefined;
                    }
            ...
            case __DYNAMO_EXPRESSION_TOKEN.__SYMBOL: //Symbol
                #region
                ...
                if (_byte == 61) //=
                {
                    ...
                    ||  (_last_byte == 62)) // >=
                    {
                        _next_state = __DYNAMO_EXPRESSION_TOKEN.__SYMBOL; //Symbol
                    }
                }
        ...
        if (_change_state && (_next_state == __DYNAMO_EXPRESSION_TOKEN.__UNKNOWN))
        {
            #region
            ...
            {
                _next_state = __DYNAMO_EXPRESSION_TOKEN.__IDENTIFIER; //Word/Variable Name
            }
            ...
            {
                _next_state = __DYNAMO_EXPRESSION_TOKEN.__NUMBER; //Number
            }
            else if (_byte == 47) // /
            {
                _next_state = __DYNAMO_EXPRESSION_TOKEN.__SYMBOL; //Symbol
        ...
        _state = _next_state;
        if (_state == __DYNAMO_EXPRESSION_TOKEN.__NULL) break;
        _last_byte = _byte;
    ...
    {
        __DynamoError("No valid expression tokens found (", _string, ")");
    }
    else
    {
        if (array_length(_tokens) > 1)
        {
            __DynamoError("Expression could not be fully resolved into a single token (", _string, ")");
        }
        else
        {
            return _tokens[0];
...
function __ChatterboxCompileExpression(_source_array)
{
    ...
    var _o = 0;
    repeat(array_length(global.__dynamoExpressionOpList))
    {
        var _operator = global.__dynamoExpressionOpList[_o];
